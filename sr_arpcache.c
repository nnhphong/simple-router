   ...
   if (difftime(now, req->sent) > 1.0) {
      if (req->times_sent >= 5) {
         ...
         for (pkt = req->packets; pkt != NULL; pkt = pkt->next)
            ...
            sr_send_icmp_error(sr, pkt->buf, pkt->len, pkt->iface,
...
void sr_send_arp(struct sr_instance *sr, enum sr_arp_opcode op_code,
                 uint32_t dest_ip, unsigned char *dest_mac_addr) {
   struct sr_rt *route = (struct sr_rt *)sr_get_matching_route(sr, dest_ip);
