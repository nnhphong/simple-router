...
void print_hdrs(uint8_t *buf, uint32_t length) {
    ...
    if (length < minlength) {
        fprintf(stderr,
                "Failed to print ETHERNET header, insufficient length\n");
        return;
    }
    ...
    if (ethtype == ethertype_ip) { /* IP */
        ...
        if (ip_proto == ip_protocol_icmp) { /* ICMP */
            ...
            if (length < minlength)
                fprintf(stderr,
                        "Failed to print ICMP header, insufficient length\n");
            else
                print_hdr_icmp(buf + sizeof(sr_ethernet_hdr_t) +
                               sizeof(sr_ip_hdr_t));
        }
        ...
        if (length < minlength)
            fprintf(stderr,
                    "Failed to print ARP header, insufficient length\n");
        else
